...
local translations = {en="Rotorflight"}
...
local function create()
    ...
    protocol = assert(loadScript("/scripts/RF/protocols.lua"))()
    radio = assert(loadScript("/scripts/RF/radios.lua"))().msp
    assert(loadScript(protocol.mspTransport))()
    assert(loadScript("/scripts/RF/MSP/common.lua"))()
...
local function wakeup(widget)
    ...
    elseif uiState == uiStatus.init then
        ...
        init = init or assert(loadScript("/scripts/RF/ui_init.lua"))()
        if lastEvent == EVT_VIRTUAL_EXIT then
			lastEvent = nil
			lcd.invalidate()
            callCreate = true
        ...
        init = nil
        PageFiles = assert(loadScript("/scripts/RF/pages.lua"))()
        invalidatePages()
    ...
    elseif uiState == uiStatus.pages then
        ...
        if not Page then
            Page = assert(loadScript("/scripts/RF/PAGES/"..PageFiles[currentPage].script))()
            collectgarbage()
        end
...
local icon = lcd.loadMask("/scripts/RF/RF.png")
